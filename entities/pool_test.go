...
func TestNewPool(t *testing.T) {
	_, err := NewPool(USDC, entities.WETH9[3], constants.F3000, utils.EncodeSqrtRatioX96(constants.One, constants.One), big.NewInt(0), 0, nil, false, nil)
	assert.ErrorIs(t, err, entities.ErrDifferentChain, "cannot be used for tokens on different chains")

	_, err = NewPool(USDC, entities.WETH9[1], 1e6, utils.EncodeSqrtRatioX96(constants.One, constants.One), big.NewInt(0), 0, nil, false, nil)
	assert.ErrorIs(t, err, ErrFeeTooHigh, "fee cannot be more than 1e6'")

	_, err = NewPool(USDC, USDC, constants.F3000, utils.EncodeSqrtRatioX96(constants.One, constants.One), big.NewInt(0), 0, nil, false, nil)
	assert.ErrorIs(t, err, entities.ErrSameAddress, "cannot be used for the same token")

	_, err = NewPool(USDC, entities.WETH9[1], constants.F3000, utils.EncodeSqrtRatioX96(constants.One, constants.One), big.NewInt(0), 1, nil, false, nil)
	assert.ErrorIs(t, err, ErrInvalidSqrtRatioX96, "price must be within tick price bounds")

	_, err = NewPool(USDC, entities.WETH9[1], constants.F3000, new(big.Int).Add(utils.EncodeSqrtRatioX96(constants.One, constants.One), big.NewInt(1)), big.NewInt(0), -1, nil, false, nil)
	assert.ErrorIs(t, err, ErrInvalidSqrtRatioX96, "price must be within tick price bounds")

	_, err = NewPool(USDC, entities.WETH9[1], constants.F3000, utils.EncodeSqrtRatioX96(constants.One, constants.One), big.NewInt(0), 0, nil, false, nil)
	assert.NoError(t, err, "works with valid arguments for empty pool medium fee")

	_, err = NewPool(USDC, entities.WETH9[1], constants.F500, utils.EncodeSqrtRatioX96(constants.One, constants.One), big.NewInt(0), 0, nil, false, nil)
	assert.NoError(t, err, "works with valid arguments for empty pool low fee")

	_, err = NewPool(USDC, entities.WETH9[1], constants.F10000, utils.EncodeSqrtRatioX96(constants.One, constants.One), big.NewInt(0), 0, nil, false, nil)
	assert.NoError(t, err, "works with valid arguments for empty pool high fee")
}

func TestGetAddress(t *testing.T) {
	addr, _ := GetAddress(USDC, DAI, constants.F500, "")
...
func TestToken0(t *testing.T) {
	pool, _ := NewPool(USDC, DAI, constants.F500, utils.EncodeSqrtRatioX96(constants.One, constants.One), big.NewInt(0), 0, nil, false, nil)
	assert.Equal(t, pool.Token0, DAI, "always is the token that sorts before")

	pool, _ = NewPool(DAI, USDC, constants.F500, utils.EncodeSqrtRatioX96(constants.One, constants.One), big.NewInt(0), 0, nil, false, nil)
	assert.Equal(t, pool.Token0, DAI, "always is the token that sorts before")
}
...
func TestToken0Price(t *testing.T) {
	...
	pool0, _ := NewPool(USDC, DAI, constants.F500, utils.EncodeSqrtRatioX96(a1, a2), big.NewInt(0), r, nil, false, nil)
	assert.Equal(t, pool0.Token0Price().ToSignificant(5), "1.01", "returns price of token0 in terms of token1")

	pool1, _ := NewPool(DAI, USDC, constants.F500, utils.EncodeSqrtRatioX96(a1, a2), big.NewInt(0), r, nil, false, nil)
...
func TestChainID(t *testing.T) {
	pool0, _ := NewPool(USDC, DAI, constants.F500, utils.EncodeSqrtRatioX96(constants.One, constants.One), big.NewInt(0), 0, nil, false, nil)
	assert.Equal(t, pool0.ChainID(), uint(1), "returns the token0 chainId")

	pool1, _ := NewPool(DAI, USDC, constants.F500, utils.EncodeSqrtRatioX96(constants.One, constants.One), big.NewInt(0), 0, nil, false, nil)
...
func newTestPool() *Pool {
	ticks := []Tick{
		{
			Index:          NearestUsableTick(utils.MinTick, constants.TickSpacings[constants.F500]),
			LiquidityNet:   OneEther,
			LiquidityGross: OneEther,
		},
		{
			Index:          NearestUsableTick(utils.MaxTick, constants.TickSpacings[constants.F500]),
			LiquidityNet:   new(big.Int).Mul(OneEther, constants.NegativeOne),
			LiquidityGross: OneEther,
		},
	}

	p, err := NewTickListDataProvider(ticks, constants.TickSpacings[constants.F500])
	if err != nil {
		panic(err)
	}

	pool, err := NewPool(USDC, DAI, constants.F500, utils.EncodeSqrtRatioX96(constants.One, constants.One), OneEther, 0, p, false, nil)
	if err != nil {
		panic(err)
	}
	return pool
