...
func TestComputePoolAddress(t *testing.T) {
	...
	result, err := ComputePoolAddress(factoryAddress, tokenA, tokenB, constants.F500, "")
	if err != nil {
		panic(err)
	}
	...
	resultA, err := ComputePoolAddress(factoryAddress, USDC, DAI, constants.F500, "")
	if err != nil {
		panic(err)
	}
	resultB, err := ComputePoolAddress(factoryAddress, DAI, USDC, constants.F500, "")
	if err != nil {
		panic(err)
	}
