...
var (
	...
	pool_0_1_medium, _ = entities.NewPool(token0, token1, constants.F3000, utils.EncodeSqrtRatioX96(constants.One, constants.One), big.NewInt(0), 0, nil, nil)
	pool_1_2_low, _    = entities.NewPool(token1, token2, constants.F500, utils.EncodeSqrtRatioX96(constants.One, constants.One), big.NewInt(0), 0, nil, nil)
	pool_0_weth, _     = entities.NewPool(token0, weth, constants.F3000, utils.EncodeSqrtRatioX96(constants.One, constants.One), big.NewInt(0), 0, nil, nil)
	pool_1_weth, _     = entities.NewPool(token1, weth, constants.F3000, utils.EncodeSqrtRatioX96(constants.One, constants.One), big.NewInt(0), 0, nil, nil)
	...
	makePool = func(token0, token1 *core.Token) *entities.Pool {
		pool, _ := entities.NewPool(token0, token1, feeAmount, sqrtRatioX96, liquidity, tick, p, nil)
		return pool
	}
)
